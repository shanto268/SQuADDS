...
class TestGeometricEncoder:
    def test_output_shape(self):
        enc = GeometricEncoder(vocab_size=50, embed_dim=16, out_dim=32, k_max=10)
    ...
    def test_pad_masking(self):
        ...
        # Outputs should differ because one has a real key contributing
...
class TestNodeEncoder:
    def test_output_shape(self):
        ...
        ports = tf.random.normal((batch, 4))
    ...
    def test_gradient_flow(self):
        ...
        ports = tf.constant([[1, 0, 0, 0], [0, 1, 0, 0]], dtype=tf.float32)
        with tf.GradientTape() as tape:
            out = enc(ls, kids, vals, area, peri, ports)
            loss = tf.reduce_sum(out)
        grads = tape.gradient(loss, enc.trainable_variables)
